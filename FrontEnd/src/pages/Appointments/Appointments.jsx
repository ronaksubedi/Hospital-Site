...
const Appointments = () => {
  ...
  const [form, setForm] = useState({ doctor: preselectedDoctor || "", service: "", date: "", timeSlot: "", notes: "" });
  ...
  return (
    <div>
      <div style={{ background: "linear-gradient(to right, #EFF6FF, #DBEAFE)", padding: "60px 24px", textAlign: "center" }}>
        <p style={{ color: "#3B82F6", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "13px", marginBottom: "12px" }}>Schedule a Visit</p>
        <h1 style={{ fontSize: "42px", fontWeight: "800", color: "#111827", marginBottom: "16px" }}>Book an Appointment</h1>
        <p style={{ color: "#6B7280", maxWidth: "500px", margin: "0 auto" }}>Choose your doctor and preferred time slot.</p>
      </div>

      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "60px 24px 80px" }}>
        <div style={{ background: "white", borderRadius: "16px", padding: "40px", boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
          {success && <div style={{ background: "#D1FAE5", color: "#065F46", padding: "16px", borderRadius: "8px", marginBottom: "24px", fontSize: "14px", fontWeight: "500" }}>✅ {success}</div>}
          {error && <div style={{ background: "#FEE2E2", color: "#991B1B", padding: "16px", borderRadius: "8px", marginBottom: "24px", fontSize: "14px" }}>{error}</div>}
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <div>
                ...
                <input type="date" name="date" value={form.date} onChange={handleChange} required min={new Date().toISOString().split("T")[0]} style={inputStyle} />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea name="notes" value={form.notes} onChange={handleChange} placeholder="Describe your symptoms..." rows={4} style={{ ...inputStyle, resize: "vertical" }} />
            </div>
