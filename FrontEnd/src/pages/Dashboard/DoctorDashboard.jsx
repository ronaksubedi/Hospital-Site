...
const DoctorDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [aptsRes, doctorsRes] = await Promise.all([
        API.get("/api/appointments/doctor", { headers }),
        API.get("/api/doctors", { headers }),
      ]);
      setAppointments(aptsRes.data.appointments);
      const myProfile = doctorsRes.data.doctors.find(d => d.email === user.email);
  ...
  const handleToggle = async () => {
    ...
    try {
      const res = await API.patch(`/api/doctors/${doctorProfile._id}/availability`, {}, {
    ...
    <div style={{ background: "#F8FAFF", minHeight: "100vh" }}>
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "40px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "32px" }}>
          <div>
            <h1 style={{ fontSize: "28px", fontWeight: "800", color: "#111827", marginBottom: "4px" }}>Doctor Dashboard</h1>
          ...
          {doctorProfile && (
            <div style={{ background: "white", borderRadius: "12px", padding: "16px 24px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <button onClick={handleToggle} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", alignItems: "center" }}>
                {doctorProfile.available ? <ToggleRight size={36} color="#10B981" /> : <ToggleLeft size={36} color="#9CA3AF" />}
              </button>
        ...
        {toggleMsg && <div style={{ background: "#D1FAE5", color: "#065F46", padding: "12px 16px", borderRadius: "8px", marginBottom: "24px", fontSize: "14px" }}>{toggleMsg}</div>}

        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "20px", marginBottom: "32px" }}>
          {[
            { label: "Total Appointments", value: appointments.length, color: "#1B2F6E" },
            { label: "Pending", value: pending, color: "#92400E" },
            { label: "Confirmed", value: confirmed, color: "#065F46" },
            { label: "Completed", value: completed, color: "#1E40AF" },
          ].map((stat, i) => (
              ...
              <div key={apt._id} style={{ padding: "20px 24px", borderBottom: "1px solid #F9FAFB", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
                  ...
                  <div>
                    ...
                    {apt.notes && <p style={{ color: "#9CA3AF", fontSize: "12px", marginTop: "4px", fontStyle: "italic" }}>"{apt.notes}"</p>}
                  </div>
                </div>
