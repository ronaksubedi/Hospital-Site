...
const Doctors = () => {
  ...
  useEffect(() => {
    setLoading(true);
    API.get(`/api/doctors?search=${search}`)
  ...
  return (
    <div>
      <div style={{ background: "linear-gradient(to right, #EFF6FF, #DBEAFE)", padding: "60px 24px", textAlign: "center" }}>
        <p style={{ color: "#3B82F6", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "13px", marginBottom: "12px" }}>Trusted Care</p>
        <h1 style={{ fontSize: "42px", fontWeight: "800", color: "#111827", marginBottom: "16px" }}>Our Doctors</h1>
        <p style={{ color: "#6B7280", maxWidth: "500px", margin: "0 auto" }}>Meet our team of experienced and dedicated medical professionals.</p>
      </div>
              ...
              <Link to={`/doctors/${doctor._id}`} key={doctor._id} style={{ textDecoration: "none" }}>
                ...
                >
                  ...
                  <div style={{ padding: "20px", textAlign: "center" }}>
                    ...
                    <p style={{ color: "#3B82F6", fontSize: "14px", fontWeight: "500", marginBottom: "8px" }}>{doctor.specialty}</p>
                    <p style={{ color: "#6B7280", fontSize: "13px", marginBottom: "12px" }}>{doctor.experience} years experience</p>
                    <span style={{ display: "inline-block", padding: "4px 12px", borderRadius: "999px", fontSize: "12px", fontWeight: "600", background: doctor.available ? "#D1FAE5" : "#FEE2E2", color: doctor.available ? "#065F46" : "#991B1B" }}>
                      {doctor.available ? "Available" : "Unavailable"}
                    </span>
                    {doctor.ratingCount > 0 && (
                      <p style={{ color: "#F59E0B", fontSize: "13px", marginTop: "8px" }}>★ {doctor.rating} ({doctor.ratingCount} reviews)</p>
                    )}
                  </div>
                </div>
              </Link>
