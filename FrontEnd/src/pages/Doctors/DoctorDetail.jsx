...
import { Star, Award, Phone, Mail, ArrowLeft } from "lucide-react";
...
const DoctorDetail = () => {
  ...
  const handleRate = async (value) => {
    if (!token) { setRatingMsg("Please login to rate this doctor."); return; }
    try {
      const res = await API.post(`/api/doctors/${id}/rate`, { rating: value }, {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "40px" }}>
          <div>
            ...
            <Link to={"/appointments?doctor=" + doctor._id}
              style={{ display: "block", background: "#1B2F6E", color: "white", padding: "14px", borderRadius: "12px", textAlign: "center", textDecoration: "none", fontWeight: "600", fontSize: "15px" }}
            >
              Book Appointment
            </Link>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            <div style={{ background: "white", borderRadius: "16px", padding: "32px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
              ...
              <p style={{ color: "#6B7280", lineHeight: "1.8", fontSize: "15px" }}>{doctor.bio || "No bio available."}</p>
            </div>
            ...
            <div style={{ background: "white", borderRadius: "16px", padding: "32px", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
              ...
              <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} size={32} style={{ cursor: "pointer" }}
              ...
              {ratingMsg && <p style={{ color: "#3B82F6", fontSize: "14px", fontWeight: "500" }}>{ratingMsg}</p>}
            </div>
          </div>
        </div>
      </div>
